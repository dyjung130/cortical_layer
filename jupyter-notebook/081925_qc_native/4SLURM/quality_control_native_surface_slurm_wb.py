...
def main():
    ...
    subject_wb_path = os.path.join(wb_surfaces_path, subject_name, f'{int(params["spacing_mm"]*1000)}um_method{params["dist_method"]}')
    ...
    try:
        ...
        for layer_type in ['inf']: #just need 'inf' for now 090425 DJ
            ...
            raw, le_data, le_data_norm, dist_array, ap_order = generate_layer_intensity_profile_wb(
                vol_path, layer_type,
                surf_norm_path,
                surf_coord_path,
                save_path_subject, 
